...
@WebServlet(urlPatterns ="/table-alliesUboat")

public class AlliesRefreshUboatScreen extends HttpServlet {
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws IOException {
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            SingleUBoatEntry CurrUboat = manager.getContest(alliesbattle);
            List<DetailsAllies> detailsList = new ArrayList<>();
            if (CurrUboat!=null) {

                if (!alliesbattle.isEmpty()) {
                    int x = 100;
                }
